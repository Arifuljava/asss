...
public class SomitiAdapter extends RecyclerView.Adapter<SomitiAdapter.myview> {
    public List<AddSomitimodel> data;
    FirebaseFirestore firebaseFirestore;
    FirebaseAuth firebaseAuth;
    String detecor,gettingtext;

    public SomitiAdapter(List<AddSomitimodel> data,String detecor, String gettingtext) {
        this.data = data;
    ...
    public myview onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(parent.getContext()).inflate(R.layout.somitilayout,parent,false);
    ...
    public void onBindViewHolder(@NonNull myview holder, final int position) {
        ...
        holder.lastkisti.setText("সমিতির নাম: "+data.get(position).getSomitiname());
        holder.kinadam.setText("সভাপতির নাম : "+data.get(position).getSovapoti());
        holder.logout.setText(gettingtext);
        holder.card_view8.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (Integer.parseInt(detecor)==1) {
                    ...
                    bb.setTitle("নিশ্চিতকরণ")
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            firebaseFirestore.collection("SomitiList")
                                    ...
                                    .collection("List")
                                    .document(""+data.get(position).getSomitiname().toString().toLowerCase())
                                    .get()
                                            ...
                                            if (task.isSuccessful()) {
                                                if (task.getResult().exists()) {
                                                    firebaseFirestore.collection("SomitiList")
                                                            .document(firebaseAuth.getCurrentUser().getEmail())
                                                            .collection("List")
                                                            .document(""+data.get(position).getSomitiname().toString().toLowerCase()).delete()
                    ...
                    Intent intent=new Intent(v.getContext(),Add_member.class);
                    intent.putExtra("mm",data.get(position).getSomitiname().toString().toLowerCase());
