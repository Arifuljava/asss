...
public class AlgebraRatio extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_algebra_ratio);
        Toolbar toolbar = findViewById(R.id.toolbar);

        toolbar.setTitle("Ratio");
        ...
        String[] textSizes = getResources().getStringArray(R.array.ration);
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        if (parent.getId() == R.id.spinner3) {
            ...
            if (valueFromSpinner.equals("Ratio")) {
                //hintss.setText("A - B% = X");
                if (TextUtils.isEmpty(koto_point.getText().toString())||TextUtils.isEmpty(people.getText().toString())) {
                    Toasty.info(getApplicationContext(),"A এবং B মান লিখুন",Toasty.LENGTH_SHORT,true).show();
                    return;
                }
                else {
            ...
            else  if (valueFromSpinner.equals("Denominator")) {
                // hintss.setText("A + B% = X");
